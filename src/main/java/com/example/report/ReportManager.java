...
public class ReportManager {
    ...
    public boolean deleteReport(int id) {
        ...
        if (removed != null) {
            saveReports();
            // 通知举报人举报已被删除
            notifyReporter(removed, "deleted-notify");
            return true;
        }
        return false;
    }
    ...
    public void markAsHandled(int id) {
        ...
        if (report != null) {
            ...
            saveReports();
            // 通知举报人举报已被处理
            notifyReporter(report, "handled-notify");
        }
    }
     ...
     * 通知举报人
     * @param report 举报数据
     * @param messageKey 消息键（如 "handled-notify" 或 "deleted-notify"）
     */
    private void notifyReporter(ReportData report, String messageKey) {
        ...
        message = message.replace("{target}", report.getTargetName())
                        .replace("{reason}", report.getReason())
                        .replace("{id}", String.valueOf(report.getId()));
        
        reporter.sendMessage(plugin.getMessageUtils().colorize(plugin.getPrefix() + message));
    }
