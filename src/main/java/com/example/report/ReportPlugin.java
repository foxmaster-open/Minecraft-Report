...
public class ReportPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("^_^ Report 2.0 启动了 ^_^ ");
        getLogger().info("加载 " + reportReasons.size() + " 个举报原因");
    }
    
    @Override
    public void onDisable() {
        getLogger().info("-_- 真的...禁用了...");
    }
    ...
    private void loadReportReasons() {
        ...
        if (reportReasons.isEmpty()) {
            ...
            reportReasons.add("&b偷窃/抢夺");
            reportReasons.add("&d语言辱骂");
            reportReasons.add("&5利用BUG");
            reportReasons.add("&3其他违规");
        }
    }
    ...
    public boolean isNotifyReporter() {
        return config.getBoolean("notify-reporter", true);
    }
}
